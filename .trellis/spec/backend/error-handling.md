...
Selection uncertainty is a returned union, not an exception: `src/core/select-installation.ts` yields `selected`, `needs-input`, or `no-match`, with all states exercised in `test/release-page.test.tsx`.
