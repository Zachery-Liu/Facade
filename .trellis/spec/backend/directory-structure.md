...
Examples: `src/cli/index.ts` is the executable boundary, `src/core/select-installation.ts` is dependency-free domain logic, and `src/manifest/release-page-manifest.ts` owns the shared runtime contract. Tests mirror behavior in `test/`.
