...
These commands are the local equivalents of the main CI checks. The `minimum-node` job runs the same project against the package's declared minimum supported Node.js version. CodeQL and Dependency Review run on GitHub for Pull Requests.
...
| `CI` | `minimum-node` | The project failed on the package's minimum supported Node.js version |
...
- Required status checks use the nine check names listed above and must pass against an up-to-date branch.
...
4. Verify all nine required status checks listed above are present and that the strict up-to-date policy is enabled.
